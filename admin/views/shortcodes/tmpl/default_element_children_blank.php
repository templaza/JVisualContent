...
$bool = $this -> children;
?>
<div class="tz_column_container tz_container_for_children<?php if(!$bool) echo ' tz_empty-container fancybox';?><?php echo (!preg_match_all('/.*?(\[loop\].*?\[\/type\].*?\[\/loop\]).*?/msi',
        $this -> item -> html) && preg_match_all('/.*?(\[\/type\]).*?/msi',$this -> item -> html))?' jscontent_element_container':'';?>"
     data-fancybox-href="<?php echo $link;?>">
    <?php if($bool): echo '[/type]';else:?>

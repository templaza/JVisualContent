...
<div class="content_element tz_element-item tz_sortable"
    ...
    <div class="content_element <?php echo preg_match_all('/.*?(\[\/type\]).*?/msi',$html)?'tz_element-item ':'';?>tz_sortable"
         ...
         data-element_id="<?php echo $this -> item -> id;?>">
        <div class="tz_controls">
            <div class="tz_controls-cc jvc_btn-group jvc_btn-group-xs<?php echo preg_match_all('/.*?(\[type\].*?\[\/type\]).*?/msi',$html)?' tz_controls-out-tc tz_controls-content-widget':'';?>">
                <a class="tz_control-btn tz_element-name element-move jvc_btn jvc_btn-success jvc_btn-xs">
                    <span title="<?php echo JText::sprintf('COM_JVISUALCONTENT_DRAG_MOVE_ITEM',$this -> item -> title);?>" data-toggle="jvc_tooltip"
                          class="tz_btn-content">
                    <?php
                    if($classIcon = $this -> item -> class_icon):?>
                ...
                <?php if(preg_match_all('/.*?(\[\/type\]).*?/msi',$html)):?>
                <a title="<?php echo JText::sprintf('COM_JVISUALCONTENT_PREPEND_ITEM',$this -> item -> title);?>" href="javascript:" data-toggle="jvc_tooltip"
                   data-fancybox-href="<?php echo $link;?>"
                   class="jvc_btn jvc_btn-success tz_control-btn tz_control-btn-prepend fancybox">
                        ...
                        if(preg_match_all('/\[loop\].*?(<[a-z]+[1-6]?.*?>).*?\[\/loop\]/msi',$lp,$matches)){
                            ...
                            if($i == (count($loop) - 1)){
                                ...
                                if(!preg_match('/aria-control\=["|\'].*?["|\']/msi',$lp)){
                                    $str = preg_replace('/^(<[a-z]+[1-6]?)(.*?\>)/msi','$1 aria-control="'.$modelIds -> tz_element.'-'.$i.'" $2',$lp);
//                                    $str = preg_replace('/^(<[a-z]+[1-6]?)(.*?\>)/msi','$1 aria-control="[modelid][/modelid]" $2',$lp);
                                }else{
                                    $str = preg_replace('/^(<[a-z]+[1-6]?.*?aria-control\=["|\'])(.*?)/msi','$1 '.$modelIds -> tz_element.'-'.$i.' $2',$lp);
//                                    $str = preg_replace('/^(<[a-z]+[1-6]?.*?aria-control\=["|\'])(.*?)/msi','$1 [modelid][/modelid] $2',$lp);
                    ...
                    if(preg_match_all('/(\[\/type\])/msi',$html,$typeids)){
                        if(count($loop) > 1){
                            foreach($loop as $j => $lp){
                                $el_children                = preg_replace('/(\[\/type\])/msi',$this -> loadTemplate('element_children'),$lp);
                                $el_children                = str_replace(array('[loop]','[/loop]'),'',$el_children);
                                $this -> elementItem[$j]    = $el_children;
                            }
                        }else{
                            $el_children            = preg_replace('/(\[\/type\])/msi',$this -> loadTemplate('element_children'),$loop);
                            $el_children             = str_replace(array('[loop]','[/loop]'),'',$el_children);
                            $this -> elementItem    = $el_children;
                        }
                    }
                }
            ...
            if(preg_match_all('/(\[typeid[0-9+]?\].*?\[\/typeid[0-9+]?\])/msi',$html,$typeids)){
                ...
                if(preg_match_all('/(\[\/typeid[0-9+]?\])/msi',$html,$typeids)){
                    $typeids    = $typeids[count($typeids) - 1];
                }else{
                    $typeids    = array();
                }
            ...
            if(count($typeids)){
                ...
                foreach($typeids as $typeid){
                    $html   = preg_replace('/('.str_replace(array('[',']','/'),array('\\[','\\]','\\/'),$typeid).')/msi',time().'-0-'.$count,$html);
                    $count ++;
                }
            }
//            if(preg_match_all('/(\[typeid[0-9+]?\].*?\[\/typeid[0-9+]?\])/msi',$html,$typeids)){
...
//            }
            ...
            if(preg_match_all('/.*?(\[loop\].*?\[\/type\].*?\[\/loop\]).*?/msi',$this -> item -> html)){
                $html   = preg_replace('/(\[\/type\])/msi',str_replace('[modelid][/modelid]', $modelIds -> tz_element,$this -> loadTemplate('element_children')),$html);
                $html   = str_replace('[modelid][/modelid]', $modelIds -> tz_element,$html);
            }else{

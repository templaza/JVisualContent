...
$modelId    = $this -> model_ids -> tz_element;
...
if($this -> item && $this -> item -> html):
    ...
    if(isset($this -> item) && $this -> item && isset($this -> item -> id)){
        if(preg_match_all('/.*?(\[loop\].*?\[type\].*?\[\/type\].*?\[\/loop\]).*?/msi',$this -> item -> html)){
            $filter_link    = '&element_id='.$this -> item -> id;
        }elseif(preg_match_all('/.*?(\[type\].*?\[\/type\]).*?/msi',$this -> item -> html)){
...
<script id="tmpl-jscontent-new-element-<?php echo $this -> item -> id;?>" type="text/html">
    ...
    <?php else:?>
    <div class="content_element tz_sortable<?php echo preg_match_all('/.*?(\[\/type\]).*?/msi',$this -> item -> html)?' tz_element-item ':'';?>"
         data-model-id="<?php echo $modelId;?>"
         data-element_type="<?php echo $this -> item -> name;?>"
         data-element_id="<?php echo $this -> item -> id;?>">
            <div class="tz_controls">
                <div class="tz_controls-cc jvc_btn-group jvc_btn-group-xs<?php echo preg_match_all('/.*?(\[\/type\]).*?/msi',$html)?' tz_controls-out-tc tz_controls-content-widget':'';?>">
                    ...
                    <?php if(preg_match_all('/.*?(\[\/type\]).*?/msi',$html)):
                        ?>
                        <a  href="javascript:" data-toggle="jvc_tooltip"
                            title="<?php echo JText::sprintf('COM_JVISUALCONTENT_PREPEND_ITEM',JText::_('COM_JVISUALCONTENT_SECTION'));?>"
                            data-fancybox-href="<?php echo $link;?>"
                            class="jvc_btn jvc_btn-success tz_control-btn tz_control-btn-prepend tz_edit fancybox">
